const CACHE_NAME='orion-armonizacion-v1.6.9';
const SHELL=[
  ...
  './male-atlas-safety-v168.css','./male-atlas-safety-v168.js','./tab-stability-v169.js','./manifest.webmanifest',
...
self.addEventListener('activate',event=>{
  event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('orion-armonizacion-')&&key!==CACHE_NAME).map(key=>caches.delete(key)))));
  self.clients.claim();
});
