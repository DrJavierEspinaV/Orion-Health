const CACHE_NAME='orion-dental-app-v1.4.4-r20';
const APP_SCOPE='./';
const SHELL=[
...
self.addEventListener('fetch',event=>{
  ...
  if(isArmonizacionBase){
    event.respondWith(
      fetch(APP_SCOPE+'modules/armonizacion/registro-v147.html',{cache:'no-store'})
        .catch(()=>caches.match(APP_SCOPE+'modules/armonizacion/registro-v147.html'))
    );
    return;
  }
